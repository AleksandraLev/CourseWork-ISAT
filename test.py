...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text:
        ...
        # Вместо изменения update.message.text, передаем текст в handle_message
        # Создаем новый объект Message с текстом и передаем его
        emulated_message = Message(
            message_id=update.message.message_id,
            date=update.message.date,
            chat=update.message.chat,
            text=text
        )
        # Создаем объект обновления с fake_message
        emulated_update = Update(update.update_id, message=emulated_message)

        # Передаем fake_update в handle_message
        await handle_message(emulated_update, context)
    else:
